    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <div>
          ...
          <h2 className="text-2xl font-semibold text-gray-900">Create MCQ</h2>
        </div>
        ...
        <div className="bg-white shadow-sm rounded-lg border border-gray-200 p-6 space-y-4">
          ...
          {mcqs && (
            <div className="mt-6 p-4 bg-gray-50 rounded-lg">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Generated MCQs</h3>
              <div className="space-y-4">
                {Array.isArray(mcqs) ? (
                  mcqs.map((mcq, index) => (
                    <div key={index} className="p-3 bg-white rounded border border-gray-200">
                      <p className="font-medium text-gray-900 mb-2">{mcq.question || `Question ${index + 1}`}</p>
                      {mcq.options && (
                  ...
                  <pre className="text-gray-700 whitespace-pre-wrap">{JSON.stringify(mcqs, null, 2)}</pre>
                )}
              </div>
            </div>
          )}
